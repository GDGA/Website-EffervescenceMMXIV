<?php
	...
	if(validate($user_name, $password, $event_id)) {
		session_start();
		$_SESSION['user_stegolica']=$user_name;
		$_SESSION['islogin'] = 1;
		$_SESSION['isregistered_stegolica'] = 1;
		$check = 1;
	}
	...
	if($check==1){
		//Correct User	
		//echo $user_name;
		$query="SELECT * from user_info_ste where user_id='$user_name'";
		
		$result=mysql_query($query) or die("Could not retrieve database");
		
		if (mysql_num_rows($result)==0)
			$user_insert=mysql_query("INSERT user_info_ste (user_id) VALUES ('$user_name') ");
