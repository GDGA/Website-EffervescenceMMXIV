<?php 
	...
	if (isset($_SESSION['isregistered_stegolica']) && $_SESSION['isregistered_stegolica']) 
	{
	
		$query = "SELECT score FROM user_info_ste where user_id='".$_SESSION['user_stegolica']."'";
		$result=mysql_query($query);
		$x = mysql_fetch_array($result);
		$qid = $x['score'] + 1;
		
		//echo $qid;
		$query = "SELECT question,hints FROM question_info_ste where qid='{$qid}'";
    ...
    <div id="main">
    	<div style="width:960;margin:0px;">
            ...
            <div style="border-radius:15px;background-color:rgba(255,255,255,0.4);padding:25px;min-height:430px;width:600px;float:left;">
                    ...
                    <?php
						echo '<div class="post_header"><h2>Arena : Stegolica</h2></div><div style="font-size:18px; color:rgb(230,0,0);">';
						if (!isset($_SESSION['user_stegolica'])){ //Not logged in
							
							echo "You must be logged in to view the arena";
						} else if (isset($_SESSION['isregistered_stegolica']) && !$_SESSION['isregistered_stegolica']){
							...
							echo 'Q'.($qid). ': <a href="https://drive.google.com/open?id='.$question.'&authuser=0"  target="_new">Click Me </a><br/>'.($p);
							echo 
