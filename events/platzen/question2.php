<?php 
	...
	if (isset($_SESSION['isregistered_perplexuz']) && $_SESSION['isregistered_perplexuz']) 
	{
		$query = "SELECT score FROM user_info_pla where user_id='".$_SESSION['user']."'";
		$result=mysql_query($query);
		$qid=mysql_fetch_array($result)['score'] + 1;
		
		//echo $qid;
		$query = "SELECT question FROM question_info_pla where qid='{$qid}'";
...
<img src="images/logo.png" style="margin-top:-200px; padding-top:30px;margin-bottom: -248px; margin-left: -200px; height: 500px; width: 700px;"></img>
        ...
        <div id="fb"><a href="https://www.facebook.com/Effe.Platzen/app_202980683107053" target="_new"><img src="images/fb1.png" height="35px" width="35px" style="border-radius:0px;background-color:rgba(255,255,255,0);padding:2px;margin-top:10px;" title="Visit Us at Facebook"/></a></div>
        <div id="menu">
            <ul>
    	...
    	<div style="width:960;margin:0px;">
            ...
            <div style="border-radius:15px;background-color:rgba(255,255,255,0.4);padding:25px;min-height:430px;width:600px;float:left;">
                    ...
                    <?php
						...
						if (!isset($_SESSION['islogin'])){ //Not logged in
							
							echo "You must be logged in to view the arena";
						} else if (isset($_SESSION['isregistered_perplexuz']) && !$_SESSION['isregistered_perplexuz']){
							echo "You must register for the event <a href=\"http://effervescence.iiita.ac.in/register?page=login\">here.</a>";
						}
						else if ($show_ans<0){
							...
							echo 'Q'.($qid). ': <img width=" 520" id="qq" src="questions/'.$question.'.jpg" >' ;
							echo 
			...
			<div style="border-radius:15px;background-color:rgba(255,255,255,0.4);padding:25px;margin-bottom:30px;width:250px;float:right;">
				...
				<div id="serverday"><p style="font-weight:bold;">Time Remaining:</p></div><span id="servertime"></span>
			</div>
			<!-- -->
            <div style="border-radius:15px;background-color:rgba(255,255,255,0.4);padding:25px;margin-bottom:30px;width:250px;float:right;">
                ...
                <div class="lbe_box" id="lbox">
					<?php
						...
						session_start();
						if(isset($_SESSION['isregistered_perplexuz']) && $_SESSION['isregisted_perplexuz'] ) {
							echo '
								<h2>Welcome</h2>'.
								$_SESSION['user'].
            ...
            <div  style="border-radius:15px;background-color:rgba(255,255,255,0.4);padding:25px;width:250px;float:right;">
                ...
                <div class="lbe_box">
                    <h3>&bull; Event will begin on 22nd October 2013.</h3>
                    
                </div>
				
                <div class="lbe_box">
                    <h3>&bull; It will be a 2 day long event.</h3>
